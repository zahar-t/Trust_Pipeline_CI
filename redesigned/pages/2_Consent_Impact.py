"""Consent Impact page — Financial Architect design."""
...
hero_banner(
    ...
    badge_text="Compliance & Tracking",
    page="consent",
)
...
if not totals.empty and totals["total_gross"].iloc[0] and totals["total_gross"].iloc[0] > 0:
    ...
    with c3:
        metric_card("Funnel Visibility Loss", f"{100 - full_session_pct:.1f}%", accent_color="#c2410c", description="Sessions with limited tracking")
else:
...
with tab1:
    ...
    SELECT report_month AS month, consent_level,
           round(net_revenue_eur, 2) AS net_revenue
    FROM fct_consent_impact
    ORDER BY report_month, consent_level
    """)
    if not rev_by_consent.empty:
        fig = stacked_area(rev_by_consent, x="month", y="net_revenue", color="consent_level",
                           title="", color_map=CONSENT_COLORS)
...
with tab2:
    ...
    FROM fct_consent_impact GROUP BY consent_level ORDER BY consent_level
    """)
    if not funnel.empty:
        melted = funnel.melt(id_vars="consent_level", var_name="metric", value_name="rate")
        fig = grouped_bar(melted, x="consent_level", y="rate", color="metric",
                          title="", color_map=CONSENT_COLORS)
        fig.update_layout(font=dict(family="Inter"), plot_bgcolor="white")
        st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    detail = query("""
    SELECT report_month, consent_level, n_orders,
           ...
           unique_customers, total_sessions,
           round(session_conversion_rate * 100, 2) AS conversion_pct
    FROM fct_consent_impact ORDER BY report_month DESC, consent_level
    """)
    if not detail.empty:
        ...
        for _, r in detail.iterrows():
            rows.append([
                ...
                f"\u20ac{r['aov']:,.2f}",
                f"{int(r['unique_customers']):,}",
                f"{int(r['total_sessions']):,}",
                f"{r['conversion_pct']}%",
            ])
