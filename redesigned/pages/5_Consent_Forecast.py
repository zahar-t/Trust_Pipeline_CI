"""Consent Forecast page — Financial Architect design."""
...
hero_banner(
    ...
    subtitle="Scenario modelling for consent-level distribution changes and their impact on key business metrics",
        ...
        if not forecast.empty:
            ...
            if not baseline.empty:
                merged = forecast.rename(columns={"metric_value": "scenario_value"}).merge(
                    ...
                    on=["period", "metric_name"], how="left",
                )
...
with tab2:
    ...
    st.markdown(
        ...
        "Adjust the sliders to model a hypothetical consent distribution. Values must sum to <strong>100%</strong>.</p>",
    ...
    if total_pct == 100 and table_exists("fct_consent_impact"):
        avgs = query("""
            ...
            FROM fct_consent_impact GROUP BY consent_level
        """)
        ...
        if not avgs.empty:
            ...
            with m2:
                metric_card("Predicted Conversion", f"{weighted_conv * 100:.2f}%", accent_color="#dc2626", description="Weighted conversion rate")
            with m3:
                metric_card("Predicted Cart-to-Purchase", f"{weighted_cart * 100:.2f}%", accent_color="#b91c1c", description="Weighted cart completion")
